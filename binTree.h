...
public:
    ...
    BinTree(const BinTree<T> & tree) { transfer(tree); }
    ~BinTree();
    void transfer(const BinTree<T>& tree);
...
template <typename T>
inline void BinTree<T>::transfer(const BinTree<T>& tree){
...
inline void BinTree<T>::swap(BinTree<T> & m){
    int tempSize = m.BinTreeSize;
    BinTreeNode<T> * tempHeadNode = m.pHeadNode;
    m.BinTreeSize = this->size();
    m.pHeadNode = this->pHeadNode;
    this->BinTreeSize = tempSize;
    this->pHeadNode = tempHeadNode;
